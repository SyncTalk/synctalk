...
def tasks_producer(tasks, num_tasks, data, num_processes):
    ...
    for i in range(num_processes):
        tasks.put((None, None))
    num_tasks.value -= (
        1  # only if this point is reached, process finishes when all tasks are done.
    )
...
class Aligner:
    ...
    def __init__(self, options):
        ...
        self.src, self.close_src = self._inputObjectFromParameter(
            self.options["srcfile"]
        )
        self.target, self.close_target = self._inputObjectFromParameter(
            self.options["targetfile"]
        )
    ...
    def mainloop(self):
        ...
        if multiprocessing_enabled and self.options["num_processes"] > 1:
            ...
            scorers = [
                AlignMultiprocessed(tasks, self.options, scores, self.log)
                for i in range(self.options["num_processes"])
            ]
    ...
    def process(self, sourcelist, targetlist, translist1, translist2):
        ...
        for i, translist in enumerate(translist1):
            self.log(
                "computing alignment between srctotarget (file "
                + str(i)
                + ") and target text",
                1,
            )
        ...
        for i, translist in enumerate(translist2):
            self.log(
                "computing alignment between targettosrc (file "
                + str(i)
                + ") and source text",
                1,
            )
                ...
                if pair in phase2mirror:
                    phase3.append(
                        (
                            ...
                            + t
                            + " - "
                            + phase2[phase2mirror.index(pair)][1],
                        )
                    )
            multialign = phase3
    ...
    def eval_sents(self, translist, targetlist):
        ...
        for testID, testSent in enumerate(translist):
            ...
            cooked_test["guess"] = [
                max(len(test_normalized) - k + 1, 0)
                for k in range(1, self.options["bleu_ngrams"] + 1)
            ]
            ...
            ngrams_sorted = dict(
                [(x, set()) for x in range(self.options["bleu_ngrams"])]
            )
            for ngram in counts:
            ...
            for refID, (reflen, refmaxcounts, refset) in cooktarget:
                ...
                if ngrams_filtered:
                    ...
                    logbleu += min(
                        0, 1 - float(cooked_test["reflen"]) / cooked_test["testlen"]
                    )
                    ...
                    if score > 0:
                        ...
                        logbleu /= self.options["bleu_ngrams"]
                        logbleu += min(
                            0, 1 - float(cooked_test["testlen"]) / cooked_test["reflen"]
                        )
    ...
    def pathfinder(self, translist, targetlist):
        ...
        matrix = [
            [0 for column in range(len(targetlist) + 1)]
            for row in range(len(translist) + 1)
        ]
        pointers = [
            ["" for column in range(len(targetlist))] for row in range(len(translist))
        ]

        for i in range(len(translist)):
            alignments = dict(
                [(target, score) for (score, target, correct) in self.scoredict[i]]
            )
    ...
    def gapfiller(self, sourcegap, targetgap, pregap, postgap, translist, targetlist):
        ...
        if self.options["Nto1"] > 1 or "bleu1to1" in self.options["gapfillheuristics"]:
            ...
            for src in [
                j
                for i, j in enumerate(sourcegap)
                if (i < window or len(sourcegap) - i <= window)
            ]:
                Sent = translist[src]
            ...
            for target in [
                j
                for i, j in enumerate(targetgap)
                if (i < window or len(targetgap) - i <= window)
            ]:
        ...
        while sourcegap or targetgap:
            ...
            if sourcegap and self.options["Nto1"] > 1:
                ...
                if pregapsrc:
                    ...
                    if combinedID in scoredict:
                        ...
                        if (
                            newscore > oldscore
                            and newcorrect > oldcorrect
                            and newtarget == pregaptarget
                        ):
            ...
            if sourcegap and targetgap:
                ...
                if "galechurch" in self.options["gapfillheuristics"] and (
                    ...
                    or max(len(targetgap), len(sourcegap))
                    / min(len(targetgap), len(sourcegap))
                    < 2
                ):
                    tempsrcgap = []
    ...
    def gale_church(self, tempsrcgap, temptargetgap):
        ...
        pairs = sorted(
            list((align_texts(srclengths, targetlengths)[0])), key=itemgetter(0)
        )
    ...
    def addtoAlignments(self, pair, aligntype=None):
        ...
        if aligntype:
            ...
            if (
                len(src) == 1
                and len(target) == 1
                and (src[0], target[0]) in self.bleualign
            ):
    ...
    def printout(self, sourcelist, translist, targetlist):
        ...
        for j, (src, target) in enumerate([i[0] for i in self.multialign]):
            ...
            if self.options["printempty"]:
                if src[0] != lastsrc + 1:
                    sources.extend(
                        [sourcelist[ID] for ID in range(lastsrc + 1, src[0])]
                    )
                ...
                if target[0] != lasttarget + 1:
                    ...
                    targets.extend(
                        [targetlist[ID] for ID in range(lasttarget + 1, target[0])]
                    )
    ...
    def check_sentence_pair(
        self, j, src, trans, target, source_out, target_out, sentscores
    ):
    ...
    def filter_article_pairs(
        self, sources, translations, targets, sources_output, targets_output
    ):
        ...
        if articlescore2 > articlescore and self.options["filterlang"]:
            ...
            if articlescore > 0:
                articlescore = (articlescore * articlescorex * 2) / (
                    articlescore + articlescorex
                )
            before = len(self.sources_out)
    ...
    def write_filtered(self):
        ...
        bad_percentiles = []
        for i, (articlescore, articlescore2, before, after) in enumerate(
            self.finalbleu
        ):
            length = after - before
            totallength += length
    ...
    def close_file_streams(self):
        ...
        if self.close_out_bad2:
            self.out_bad2.close()
        for should_be_closed, output_stream in zip(
            self.close_srctotarget, self.srctotarget
        ):
            if should_be_closed:
                output_stream.close()
        for should_be_closed, output_stream in zip(
            self.close_targettosrc, self.targettosrc
        ):
            if should_be_closed:
                output_stream.close()
