...
class TestByEval(unittest.TestCase, Utils):
    ...
    def test_differentTypeOptions(self):
        self.main_test("differentTypeOptions", remove_file="removeTargetFile")

    def main_test(self, option_function, close_file_object=None, remove_file=None):
        ...
        for test_set, test_argument in [("eval1957", "-d"), ("eval1989", "-e")]:
            ...
            for fr_file, de_file in test_files:
                ...
                options = getattr(self, option_function)(
                    test_argument, srctotarget_file, targettosrc_file, output_path
                )
                a = Aligner(options)
                a.mainloop()
    ...
    def fileNameOptions(
        self, eval_type, srctotarget_file, targettosrc_file, output_file
    ):
    ...
    def stringOptions(self, eval_type, srctotarget_file, targettosrc_file, output_file):
        options = self.fileNameOptions(
            eval_type, srctotarget_file, targettosrc_file, output_file
        )
        options.pop("output-src")
