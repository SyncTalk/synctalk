...
const UploadPage = () => {
  ...
  const handleDrop = (event, fileType) => {
    ...
    if (file) {
      if (fileType === 'audio' && (file.type === 'audio/mp3' || file.type === 'video/mp4')) {
        setSelectedMp3Mp4File(file);
        setSelectedMp3Mp4FileName(file.name); // Set the file name
        ...
        <div className='upload-square'>
          ...
          >
            ...
            <div className='select-part'>
              ...
              />
              <div className='select-button'><label htmlFor="mp3Mp4File">Select file</label></div>
            </div>
            <div className="file-name">{selectedMp3Mp4FileName}</div> {/* Display file name */}
          </div>
          <div
            className={`three-part ${isFileSelected() ? 'file-selected' : ''}`}
